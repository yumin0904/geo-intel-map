...
| 9-A | 이벤트 스터디 / ITS | 사건 전후 비정상변동 | `SINGLE_SHOCK` (특정 날짜 단일사건) | 준실험 | ★★★ | ◐ event_study.py 구현·배선. **잔여: actor_filter 부재로 SINGLE_SHOCK 알맹이 null (위원회 A-1 IV 필터 대기)** |
| 9-B | 횡단/패널 회귀 | 고정효과 회귀 | `CROSS_SECTION` (국가간 비교·시간축 없음) | 상관~준실험 | ★★★ | ◐ panel_regression.py 구현·배선. 라이브 CROSS_SECTION 발화 검증 잔여 |
