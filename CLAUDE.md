...
**시각화 아님** — 엔진 자체의 추론 깊이를 높인다. (구 Phase 8 시각화는 Phase 9로 이동)
...
| 8-E | 비선형 검정 엔진(B안) | 비선형 변수에 선형검정 강제 | 임계회귀(TAR)·마르코프 체제전환·임계 event-study·전이 엔트로피 | 비선형 양성 증거 2건 | ⬜ 게이트: 데이터 6개월+ 누적 |

> **8-gate (v8.12.0 완료)**: 선형검정 적합성 게이트 — 체제·임계 변수를 선형 Granger 트랙에서
> 제외하고 "구조적 논증"으로 분류. null→비선형 승격(affirming-the-null) 제거. 8-E(B안)는
> 그 제외된 변수들을 *적극적으로* 비선형 검정하는 후속 사이클이다(데이터 누적 게이트).
...
### Phase 9 — 시각화 (후순위, 박사 수준 달성 후)

목표: 박사 수준으로 강화된 분석 엔진의 결과를 시각적으로 표현

**게이트**: Phase 8 완료(박사 수준 4개 게이트 충족) 후. 이 분기점이 **NEOUL 프로젝트 착수 지점**과 일치
("글 하나를 지도로 변환하는 데모"가 작동하는 시점 → 별도 레포·브랜드 분리).

| # | 항목 | 파일 | 상태 |
|---|------|------|------|
| P9-1 | 브리핑 연쇄 그래프 뷰 — `series_ref` Cytoscape 시각화 | `frontend/src/views/BriefingGraphView.js` | [ ] |
| P9-2 | 행위자 네트워크 자동 추출 — `event_refs` 공통 노드 생성 | `backend/api/library.py` 확장 | [ ] |
| P9-3 | Cascade 커버리지 갭 트리거 3종 — cyber/defense_policy/economic_coercion | `cascade_rules.yaml` / `engine.py` | [ ] |
| P9-4 | 브리핑 타임라인 뷰 | `frontend/src/views/BriefingTimelineView.js` | [ ] |

> ※ 구 P8-4(교차 인사이트 자동 생성)는 Phase 8 Cycle 8-D(문헌 공백 탐지)로 흡수됨.

### P9 설계 배경 보존 (2026-06-01 분석)

7개 브리핑 교차 검토에서 도출된 교차 인사이트 가설 — Phase 9 착수 시 활용:
