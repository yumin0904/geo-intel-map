...
|---|-------|------|---------------|----------|---------|------|
| 9-P | 토대 수리 (Pre-flight) | 라우터 착수 전 선결 결함 4종(H1추출·진단독립성·방법오선택·출력2계층) | — | — | ★★★ 최선행 | ⬜ |
| 9-0 | Method Router + 평가계층 일반화 | 데이터모양 판정→**방법집합** 디스패치 + 삼각측량 종합 + **`_classify_inference_grade`를 방법 무관 grader로 일반화**(공통 사다리 계약) | — | — | ★★★ 선행 | ⬜ |
...
② 삼각측량(수렴=신뢰도↑·발산=플래그)+FDR → ③ 신뢰도 캡(§22-B 일반화). 기존 Granger 로직은 첫 어댑터.
