...
**착수·완료 게이트 상세(표·조정 이력·범위 결정)** → 이관: `docs/archive/constitution_annex_20260712.md` ▣ Phase 8 — 완료 선언(07-12 비준)으로 이력화. 게이트 3조건·충족 실측은 상단 비준 블록이 원천.
...
#### Cycle 8-F — 음성 결과 분류·진단 엔진 (미착수 청사진 — 철학·파이프라인·진단/제안 표·데이터모델·평가기준 → `docs/archive/constitution_annex_20260712.md` ▣ 8-F. 구현 파일 `services/negative_result_triage.py` 미존재 실측 07-12)

핵심 원칙(잔류 불변식): **개선 제안은 자동 재검정이 아니라 "구조화된 진단 + 다음 검증 제안"이다 — 유의가 나올 때까지 변수·시차·지역을 자동 탐색해 발견으로 보고하는 것(garden of forking paths)이 8-F가 막는 안티패턴.** 진단코드 4종: `D1_NO_RELATION`(무관계가 정직한 결론) / `D2_NONLINEAR` / `D3_BAD_PROXY` / `D4_INSUFFICIENT`.
...
**Cycle 9-P·9-0 세부** → 이관: `docs/archive/constitution_annex_20260712.md` (9-P 토대수리 4종 결함 상세 — 현행 상태는 위 마스터표 행이 원천 · 9-0 MethodResult 공통 스키마 — 구현체 `services/methods/router.py`·`grader.py`가 진실원). **잔류 원칙 요약**: 사다리 칸은 식별전략 강도라 방법 무관 공통 축 · 결과 보고는 유의/비유의만이 아니라 **②효과 크기(실질 유의성)·③불확실성 구간(CI)·④내부 강건성(perturbation 민감도)** 3차원 동봉 — `prediction_scorer.py`가 §9-0 ②를 인용.
...
폐기된 캡 함수 코드블록 → 이관: `docs/archive/constitution_annex_20260712.md` ▣ §20-B
