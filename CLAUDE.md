...
```
현재 (v5.x): 행위자 프로파일 → 이론 매핑 → 서술적 인사이트
v4.0 목표:   행위자 프로파일 → 변수 식별 → 데이터 자동 조회 → 수치 근거 포함 인사이트
v5.0 목표:   인사이트 → 자동 가설 생성(H1/H0) → 기존 데이터 예비 검증 → 유의성 표시
v6.0 목표:   3개 이상 경쟁 이론 동시 적용 → 설명력 비교 → 우세 이론 선택 (박사 수준)
```
