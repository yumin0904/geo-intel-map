---
asset_type: theory
era: multipolar
geopol_region: south_china_sea
instrument_of_power: informational
level_of_analysis: non_state
regions:
- south_china_sea
- taiwan_strait
- east_china_sea
- bab_el_mandeb
sector_tag: gray_zone
strategic_posture: revisionist
summary: 전쟁과 평화의 경계에서 분쟁 문턱 아래를 유지하면서 점진적으로 현상을 바꾸는 전략 — 상대의 대응을 억제하면서 기정사실을 축적한다.
temporal_era: us_china_rivalry
theorists:
- Michael Mazarr
- Hal Brands
- Christopher Dougherty
theory_id: gray_zone_gray_zone_strategy
title: 회색지대 전략 (Gray Zone Strategy)
year: 2015
---

## 핵심 주장

RAND의 Michael Mazarr가 체계화한 회색지대 전략은
**분쟁의 문턱(threshold)을 의도적으로 낮게 유지**하면서
상대국의 군사 대응을 유발하지 않는 범위 안에서
전략적 이득을 점진적으로 축적하는 방법이다.

핵심 특성:
1. **모호성** — 국가 행위인지 비국가 행위인지 판단을 어렵게 만듦
2. **점진성** — 한 번의 극적 변화 대신 살라미 슬라이싱(salami slicing)
3. **복합성** — 군사·경제·외교·정보전 동시 구사
4. **문턱 관리** — 상대의 무력 대응 정당성을 항상 박탈

## 주요 학자 및 저작

이 이론의 핵심 기여자와 대표 저작:

- Mazarr, M. (2015). *Mastering the Gray Zone*. Strategic Studies Institute.
- Brands, H. (2016). "Paradoxes of the Gray Zone." *Foreign Policy Research Institute*.
- 이호령 (2021). "회색지대 전략과 한국의 대응." KINU 연구총서.

> "회색지대 행위자들은 우리의 대응 딜레마를 정확히 이해하고 이를 이용한다.
> 우리가 반응하면 그들이 원하는 구도, 반응하지 않으면 그들이 원하는 결과."
> — Hal Brands

## 현대 지정학과의 연결

### 남중국해 인공섬 구축
중국의 남사군도 산호초를 인공섬으로 전환한 작전은
회색지대 전략의 완성판이다:
- 단계마다 "군사 목적 없음" 주장
- 어선·해경선 활용으로 군사 충돌 방지
- 완공 후 군사기지화 → 이미 기정사실(fait accompli)

### 동중국해 센카쿠/댜오위다오
중국 해경선의 반복적 영해 침입은
전형적인 살라미 슬라이싱이다.
매번 수위를 약간씩 높여 일본의 군사 대응 문턱을 탐색한다.

### 후티의 홍해 전략
공식 전쟁 선포 없이 민간 상선을 공격함으로써
글로벌 해운 보험료를 상승시키고 희망봉 우회를 강제하는 것은
비국가 행위자의 회색지대 전략이다.

## Cascade 연결 포인트

| 트리거 이벤트 | 연쇄 반응 | 관련 룰 ID |
|---|---|---|
| 남중국해 군사 긴장 | LNG 가격(NG=F) 상승 | `south_china_sea_to_lng` |
| 남중국해 군사 긴장 | 방위산업(ITA) 상승 | `south_china_sea_to_defense` |
| 바브엘만데브 공격 | 유가(CL=F) 상승 | `bab_el_mandeb_tension_to_oil` |

## 학습 노트

> "회색지대 행위자들은 우리의 대응 딜레마를 정확히 이해하고 이를 이용한다.
> 우리가 반응하면 그들이 원하는 구도, 반응하지 않으면 그들이 원하는 결과."
> — Hal Brands

지도에서 ACLED 레이어의 심각도를 0으로 낮추고 '전략적 발전' 이벤트에 주목하라.
드론 공격, 무기 밀수, 해경 파견이 전부 표시된다.
그 점들을 시간순으로 보면 회색지대 전략의 살라미 슬라이싱이 보인다.

## 참고 자료

- Mazarr, M. (2015). *Mastering the Gray Zone*. Strategic Studies Institute.
- Brands, H. (2016). "Paradoxes of the Gray Zone." *Foreign Policy Research Institute*.
- 이호령 (2021). "회색지대 전략과 한국의 대응." KINU 연구총서.