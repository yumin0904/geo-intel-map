...
### ✅ eval judge 무료화 — DeepSeek(NIM) judge + Gemini 완전 제거 (v9.22.0, 2026-07-06)

- 사용자: "judge도 DeepSeek(NIM)으로 → 비용 완전 해결." `_judge_quality`를 provider 추상화
  (`_judge_via_nim`/`_judge_via_gemini`, `JUDGE_PROVIDER`·`JUDGE_MODEL` env). reasoning 모델의
  `<think>` 오염 대비 '마지막 JSON 객체' 파싱으로 변경.
- **judge 모델 실측 비교 (nk-provocation result_md 채점)**:
  - **DeepSeek-v4-pro → judge 채택**: 14.9s, 채점 3·3·4·3(엄격) — "인과 등급-동사 불일치·통제변수
    불완전" 결함 정확 지적. judge는 엄격해야 개선 신호가 나온다.
  - Qwen3-next: 3.7s이나 4·5·5·4(관대, 5점 남발) — 생성 모델이라 자기채점 편향 우려.
  - 채택 근거: **생성=Qwen3 / judge=DeepSeek 분리** → 자기채점 편향 0 + judge 엄격성 확보.
    judge 출력 짧아 15s×33=~8분 감수 가능(eval 저빈도). `.env JUDGE_PROVIDER=nim·JUDGE_MODEL=deepseek-v4-pro`.
- **효과: Gemini 완전 제거 → 생성·채점 전부 NIM 무료.** 단 과거 Gemini judge 점수(종합 3.68 등)와
  **절대 비교 불가** → DeepSeek judge로 새 baseline 재설정 후 그 기준선끼리 비교(측정 일관성 유지).
- `JUDGE_PROVIDER=gemini`로 과거 baseline 재현 폴백 유지.

### ✅ NVIDIA NIM 생성 provider 통합 — Ollama 대체 (v9.21.0, 2026-07-06)

- 배경(사용자): 로컬 Ollama(7b) 카드 품질 저하 + Gemini 503 과부하·무료티어 한도 → NIM으로
  생성 대체. NIM = build.nvidia.com 무료·OpenAI 호환·70b급.
- 구현: `api/intel_query.py` — `_nim_stream_text`(OpenAI 호환 `/chat/completions` SSE) +
  `_LLM_PROVIDER=="nim"` 분기. NIM은 대형 모델이라 압축 카드 아닌 **풀 11섹션 프롬프트**
  (Gemini와 동일). max_tokens 8192(한국어 토큰 효율 낮아 4096은 중간 절단 실측). `.env`
  `NVIDIA_API_KEY`·`NIM_MODEL`(기본 meta/llama-3.3-70b-instruct)·`NIM_BASE_URL`. **키는
  gitignore된 .env에만**(커밋 안 됨). `export_insight.py --provider nim` 지원. LLM_PROVIDER 기본 nim 전환.
- 실측: 스모크 200 OK. 호르무즈 쿼리 export **완결성 PASS**(11섹션·2카드·[주장]등급·[경쟁설명]
  예측/실측/판정 정상), 검정 p=0.803 비유의 PENDING 정상. Token-Zero 불변(산술·통계·게이트는 파이썬).
- **부수 성과**: Gemini 503으로 막혔던 **IV 구성타당도 게이트 라이브 검증을 NIM으로 완료** —
  "북한 도발" export에서 두 가설 다 `construct_validity_fail`(North Korea share=0.0) 폐기 확인.
- **모델 비교·채택 (v9.21.1)**: NIM 121모델 중 대형 후보 3종 실측 비교(호르무즈 쿼리):
  - Llama-3.3-70b: 완결성 PASS이나 [근거] '중국 45%' 출처 없는 환각.
  - **Qwen3-Next-80b-a3b-instruct → 채택**: [근거] 출처 태그 충실("592건 ACLED"·"Brent 80.22 [FRED]"·
    "EIA 추정"), [경쟁설명] 학자 귀속(Hirschman)+예측/실측/부호. MoE(active 3b)라 빠름. 완결성 PASS.
  - DeepSeek-v4-pro: 근거 최상세이나 12,575자 과다→완결성 FAIL + 매우 느림(3분+) → 부적합.
  - 채택 근거: Qwen3가 출처 충실도+완결성+속도 균형 최적. `.env NIM_MODEL=qwen/qwen3-next-80b-a3b-instruct`.
  - 지원: `export_insight.py --model` 오버라이드(비교용) + `_strip_code_fence`(Qwen이 카드를 ```로
    감싸 완결성 오판하던 것 후처리 제거). **eval judge는 Gemini 유지**(점수 비교성).

### ✅ IV 구성타당도 게이트 — "확실한 변수" 1단계 (v9.20.0, 2026-07-05)

- 목표(사용자): "추론이 확실한 절차·확실한 변수로 진행되게 하라." 문서: geo-os/docs/ENGINE_CONSTRUCT_VALIDITY.md.
- **실측이 드러낸 진짜 문제**: 7호 IV `korean_peninsula` conflict 4,758건의 실제 구성 —
  South Korea 4,672 · Japan 28 · China 2 · **North Korea 0**(ACLED 폐쇄국가 미커버), 대부분
  event_type=Protests(국내 시위). "북한 도발" 가설의 독립변수가 남한 국내 시위를 98% 세고
  있었다. 변수 통제 이전에 **변수가 질문 대상을 전혀 측정 못 하는 구성타당도 붕괴** — 그런데
  큰 n(4,758)이 그럴듯한 p=0.064를 만들었다.
- 원칙: 필터로 검정을 성립시키는 게 목표가 아니라 **변수가 질문을 못 측정한다는 사실을
  드러내는 것**(정직성>프록시의 IV판). 필터 후 표본 부족은 숨기지 말고 정직하게 폐기.
- 구현:
  - `services/methods/iv_construct.py` 신설 — `probe_event_iv`(event_archive payload의
    country·event_type 분포 집계, 순수 SQL) + `assess_construct`(IV 텍스트가 명시한 국가가
    표본에 임계 10% 미만이면 FAIL). 한/영 별칭 완전형 매칭(‘korean_peninsula’ 부분오염 방지).
  - `hypothesis_verifier.py`: verify 루프 **초입 공통 게이트**(라우팅 전 — Granger·과정추적 등
    모든 event-IV 경로 차단) + `_ROUTE_CONSTRUCT_FAIL`. C-4 skip 가드에 연동(감사 메타 보존).
  - `tests/test_iv_construct.py` 신설 — 단위(키워드·probe·assess) + 통합(실 verify_hypotheses로
    "북한 IV" → construct_validity_fail 폐기 + 오탐 0 + 감사 메타). 통과. 인접 회귀 2종 무손상.
- **추가 발견**: 과정추적 db_evidence에도 동일 오염(후프 검정 증거로 남한 시위 배치) — 공통
  게이트가 국가 명시 IV는 함께 차단. 국가 미특정 IV의 과정추적 오염은 2단계 actor 필터로.
- 미완: 라이브 export 스모크(Gemini 503 과부하) — 다음 세션. 코드 경로는 통합 테스트로 검증됨.
- 후속(낮 설계): 2단계 actor/type 실제 필터(measurable_variables.yaml) · 3단계 DV 치환 린터.

### ✅ method_result 2계층 분열 수정 — 엔진 위원회 C-4 (v9.19.1, 2026-07-05)

- 결함(위원회 반박위원 D가 코드 확정): Method Router 루프(`hypothesis_verifier.py` L923~)가
  **모든** spec에 어댑터를 호출 → Type_A 매핑 실패·방법 미구현·정량가설 없음으로 이미 단락된
  PENDING spec에도 `event_study`를 돌려 ticker 없는 spec에 **effect=None 껍데기 MethodResult**를
  기록. 1차 등급은 PENDING인데 method_result엔 event_study 결과가 실려, 소비자(export_insight→
  neoul 기사)가 '검정 결과 있음'으로 오독하던 정합성 결함.
- 수정 3점:
  - `hypothesis_verifier.py`: Method Router 루프 초입에 `_NO_TEST_ROUTES`(pending_method_unimplemented·
    pending_typeA_no_mapping·no_quantitative_hypothesis) 가드 — PENDING이면 `method_result={"skipped":True,
    "skip_reason":...}`로 두고 어댑터 skip. **PENDING → method_result null 불변식** 확립.
  - `services/methods/event_study.py`: 진입부 ticker 빈 값 이중 방어 — `_no_ticker_result()`
    (assumptions_met=False) 반환, 껍데기 생성 원천 차단.
  - `tests/test_method_result_invariant.py` 신설 — 단위(event_study ticker='') + 통합
    (verify_hypotheses 매핑실패 spec) 회귀. 통과.
- 라이브 검증: nk-defense-recheck export — 정상 PAIRED_TIMESERIES 경로 회귀 없음(granger
  effect=2.224·sig=0.064·assumptions_met=True 정상 기록).
- 문서 정정: CLAUDE.md Phase 9 표 — 9-0 ✅·9-A/9-B ◐·9-P ◐로 갱신(문서가 코드보다 뒤처져 있던
  것 해소 — 위원회 시스템위원 발견). **9-A 잔여: actor_filter 부재(A-1 IV 필터, 낮 설계 대기).**

### ✅ 헤드리스 인사이트 export — 엔진→퍼블리싱 구조화 출구 신설 (v9.19.0, 2026-07-05)

- 배경(실측): HypothesisSpec 구조화 필드(verification_status·inference_grade·exploratory·
  method_result)는 SSE `hypothesis` 이벤트에만 존재 — `/api/intel/save`는 result_md(마크다운)만
  남겨 **구조화 산출물이 저장 시 소실**되던 갭. 너울 발행 루틴(geo-os `workflows/insight_publishing.md`)이
  이 필드를 지면 confidence로 승계해야 해서 출구가 필요해짐.
- 신설 `scripts/export_insight.py`: 서버 없이 분석 1회 실행(parse_query→build_intel_context→
  _build_prompt→_stream_gemini 직접 소비 — 엔드포인트와 동일 경로, HTTP만 생략) →
  `exports/insights/<id>.insight.json` (shared/schemas **insight-v0.1** 계약, git 커밋 = 기사 근거 감사추적).
  `--provider` 기본 **gemini**(발행용 정식 — .env의 개발용 ollama보다 실행 시점 우선), `--save`로
  intel_analyses 동시 저장.
- 실측 검증: 소말릴란드-홍해 쿼리 — ollama 경로(첫 실행)에서 압축카드·[관찰] 누락으로 완결성 FAIL,
  gemini 경로 PASS(confidence 100·가설 2건 PENDING 캡처·SINGLE_SHOCK/PAIRED_TIMESERIES 시그니처
  보존·intel_analyses id=10). 스키마 검증 통과 + geo-os 변환기까지 관통.
- 경계: 이 스크립트 실행은 엔진 내부 소관(geo-os Scheduler 경계) — 외부는 insight.json만 소비.
