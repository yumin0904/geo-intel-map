...
1. **(사용자)** 공공데이터포털 가입 → 보도자료 API 키 발급 (자동승인, 즉시) — `! ` 로그인 필요
2. **(Claude)** 키 수령 후 탐색 스크립트 → "과거 깊이 / 한반도·동맹 발언 충분량" 선확인
3. 쓸 만하면 → `connectors/`로 정식 적재 + 질적 갈래(우선순위 3) 착수
4. (병행 가능) 우선순위 1·2(해석 주체 이전·탐색 라벨)는 외부 데이터 없이 착수 가능 → **선착수 후보**
...
- **`connectors/govinfo_connector.py`** 신설:
  ...
  - `online_search(query, region)`: 검색 API 온디맨드 쿼리 + DB 캐싱 → process_tracing에서 직접 호출
  - 대통령 문서계열 필터: CPD/WCPD/DCPD/PPP만 사용 (STATUTE 제외)
...
- **현재 적재**: govinfo_releases 10건 (2026년 이란 군사작전 성명 중심 — 당면 최대 사건 반영)
- **한반도 온디맨드**: 가설 분석 시 검색 API 즉시 호출 → 트럼프-김정은 정상회담 등 역사적 문서 실시간 접근
