...
#### Phase 4 착수 순서
1. ✅ **[P4-0] 국가 지정학 프로파일** — 완료 (v4.1.0)
2. ✅ **[P4-1] 실시간 소스 다변화** — ReliefWeb RSS 편입 완료 (v4.2.0)
3. ✅ **[P4-2] GDELT GKG 적재** — 완료 (v4.3.0)
4. ✅ **[P4-3] 품질 게이트 대시보드** — 완료 (v4.4.0)
...
| P4-4 | Cascade 룰 자동 후보 생성 | ⏳ Granger 6개월+ 게이트 미충족 | — |

### 다음 세션 우선순위

#### P4-4 사전 조건 확인
- Granger 분석 데이터 누적 기간: 최소 6개월치 이벤트 필요
  (현재: ACLED 2024-05 ~ 2025-05 적재 완료, 약 12개월 → **게이트 충족 가능**)
- `GET /api/cascade/correlation` 결과 재검토 후 YAML 초안 생성 착수 여부 결정

#### 선택 가능한 다음 작업
1. **P4-4 착수** — Granger 유의쌍 재스캔 후 룰 후보 YAML draft (인간 승인 필수)
2. **국가 프로파일 확충** — 15개국 → 추가국 (IDN·NGA·ZAF 등)
3. **Phase 5 설계** — 추론 지능화 로드맵 검토 (Stage 5 명분·의도 구현)
