...
| 조건 | 현재 (v8.7.0) | 목표 |
|------|------|------|
| LLM 심판 종합 | **3.57/5** | **4.2+** |
| 경쟁이론엄밀 | **3.40/5** | **4.0+** |
| Type_B 비율 | 14% ✅ | **15% 미만** |
| Granger | VERIFIED 2건 ✅ | **2건 유의 또는 구조적 설명 승격** |

> ⚠️ Granger 정직성 가드: 정직한 방법으로도 유의 안 나오면 "왜 비선형 전이인가"를 핵심 발견으로 승격. 조작 시 기각.
...
| 케이스 | 원인 |
|--------|------|
| korean_peninsula_alliance | 응답 잘림 반복 → 신뢰도 0, 전 섹션 누락 |
| china_cyber_us | 응답 잘림 반복 → 신뢰도 15, 전 섹션 누락 |
| china_ai_export_ban | [검증포인트][문헌공백] 섹션 누락 |
| india_indo_pacific_balancing | [검증포인트][문헌공백] 섹션 누락 |
| taiwan_liberal_vs_realist | [단계5] 누락 · 질적 2.5/5 (경쟁이론 수치 비교 미충족) |

**진단**: 이론 51개 적재는 경쟁이론 수치비교 충족률(89%)에 기여하나, 경쟁이론엄밀(3.40) 점수 자체는
형식 달성과 별개로 Gemini 추론 깊이 문제. 응답 잘림 2건은 서버 컨텍스트 길이 문제로 이론 DB와 무관.
