...
| ID | 항목 | 비고 |
|----|------|------|
| B3 | 조건부 Granger (통제변수 VIX·유가) → 교란 통제 | 양변량 졸업 |
| B4 | 사건→사건 Granger (EMH 회피, cascade 본질) | 선행성 칸 정당 도달 경로 |
| C1 | eval 질적 평가 (비자명성·출처 일관성) | 형식→참 |
| C2 | 케이스 30+ / N회 median | 과적합 방지 |
| A2+ | 프론트 UI 2축 표시 (사다리 뱃지) | `InsightAnalystView.js` |
