...
> (mechanism/attribution형: `china_cyber_us`·`india_indo_pacific_balancing`·`salt_typhoon`)은 아직 미포착 —
> 다른 감지 문제(별도 사이클). Gemini 본문 라이브(429 회복 시)는 선택 확인.
...
**남은 것**: ① 비-판별형 UNQUANTIFIABLE(mechanism "어떻게 회피", attribution "실패한 이유", 역량공백) 감지는 별도 문제 — 광범 정규식은 정량 케이스 오탐 위험이라 보류. ② Gemini 라이브 본문 서술(429 회복 시 선택). ③ 라이브 33케이스 eval 재측정 시 라우팅 일치율 개선 확인.
