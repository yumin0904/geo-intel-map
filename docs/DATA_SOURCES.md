...
| **GDELT 국가급 일간 카운트 — 발생지 키 (BigQuery, 정본)** | `scripts/load_gdelt_bq.py` | 2015~현재 **ActionGeo_CountryCode(사건 발생지)** × 일 집계 → `gdelt_geo_country_daily`. 행 키=(day, fips), `country_iso3`는 `config/fips_iso3.yaml` 매핑(**다대일**: RI·RB→SRB, GZ·WE→PSE — 소비자는 iso3로 GROUP BY·SUM 의무). **원본 카운트(`n_*`)와 행위자 유형 필터 카운트(`n_*_pol`, `config/gdelt_actor_types.yaml`) 병기**: 물리충돌은 `_pol`을 쓰고(CAMEO 코더가 F1 그랑프리를 '물리적 충돌'로 오탐 — 모나코 2026-06 279건 전부가 F1 기사), 시위는 원본을 쓴다(시위 주체는 민간이라 필터 시 신호 소실). 수동 백필(39.5GB 스캔/회 = 무료 1TB의 4%, ADC 사용자 계정·프로젝트 geo-intel-gdelt-2026). 미매핑 0.07%(공해·스프래틀리 등)는 iso3 NULL로 **보존**. ⚠️ §18-A: 국가급 이상 전용 + Hammond-Weidmann 보도편향 [한계] 의무. ⚠️ `goldstein_avg`는 **전쟁을 탐지하지 못한다**(우크라이나 2022-02 전면침공 월평균 +0.41 > 2021-11 평시 +0.02) — 국가-월 평균이 폭력과 격렬 외교를 상쇄. 컬럼은 보존하되 **소비 금지**(회귀 가드: `tests/test_gdelt_construct_validity.py`). 엔진수리위 2026-07-13 |
| GDELT 국가급 일간 카운트 — 행위자 국적 키 (레거시) | `scripts/load_gdelt_bq.py --target actor` | `gdelt_country_daily`. ⚠️ `country`는 `Actor1CountryCode` = **행위자 국적**이지 발생지가 아니다 — 발생지 해석에 쓰지 말 것(정본은 윗줄). 폐기하지 않은 이유: "누가 등장했나"는 "어디서 일어났나"와 별개 구성개념. **현재 소비자 없음**(2026-07-13 실측). 판례 20260709-data-audit-committee 웨이브2 |
