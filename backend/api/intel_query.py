...
⚠️ 마지막 확인: 출력에 중국어·일본어 문자가 한 글자라도 섞이면 안 된다(品类·苏丹 같은 한자 금지 — 반드시 한국어로). 영어 고유명사·티커만 예외.
"""
...
def _build_prompt(pq: ParsedQuery, context_text: str, synthesis_ctx: str) -> str:
    ...
    system_role = (
        ...
        # ── B28 전건 판정 가능성 (2026-07-14) ────────────────────────────────
        #
        # **DV는 「측정 가능 변수 메뉴」에서 강제 선택인데, IV는 "가능하면"이었다.**
        # 그 비대칭이 track record 0/0을 만들었다 — 채점된 82건(HIT 49·MISS 33)이
        # **전건이 실제로 일어났는지 판정 불가**해서 전부 무효가 됐다.
        #
        # 그리고 프롬프트가 권장하던 ①번(ACLED 분쟁 건수)은 **랙 365일이라 원리상
        # 전건 판정이 불가능하다.** 판정 불가능한 것을 권장하고 있었다.
        "★ [10-1 전건 판정 가능성 — 필수] H1을 **시장 예측**(Y가 티커)으로 계측할 경우, "
        "독립변수 X는 만기에 **«실제로 일어났는가»를 판정할 수 있어야** 한다. "
        "판정 불가능한 X는 예측을 **공허하게 참**으로 만든다 — "
        "«X가 오르면 Y가 오른다»에서 X가 안 일어났으면 그 예측은 참도 거짓도 아니다.\n"
        "   ✅ **전건으로 쓸 수 있는 것 (신선한 소스)**:\n"
        "      · **북한 도발 건수** (CSIS Beyond Parallel, 랙 11일)\n"
        "        ⚠️ **반드시 «방향» 또는 «임계»를 붙여라. 명사구는 판정 불가다.**\n"
        "        ✅ '북한 도발 건수가 직전 30일 대비 **증가**'\n"
        "        ✅ '북한 미사일 발사 건수가 월간 **2건 이상**'\n"
        "        ❌ '북한 미사일 발사 건수(월별)'  ← 방향도 임계도 없다. **참·거짓을 물을 수 없다.**\n"
        "   ⚠️ **ACLED 분쟁 건수는 전건으로 쓸 수 없다** — 학술 티어라 **365일 늦게** 도착한다. "
        "만기에 «그 창에 분쟁이 있었나»를 물으면 **데이터가 아직 없다**. "
        "(과거 시계열 Granger 검정에는 여전히 유효하다 — 그건 지난 데이터를 쓰니까.)\n"
        "   판정 가능한 X가 없으면 **그렇게 말하라.** 억지로 ACLED를 쓰면 그 예측은 만기에 "
        "«판정 불능»으로 종결되고 **track record에 들어가지 못한다.**\n\n"
...
- [최종 판정]의 **결론**은 모드와 판정을 분리해 `[확증 모드] 판정: 지지/반증/불확실` 형식으로 써라.
  모드는 절차 선언(데이터 보기 전 가설 선언)이지 결과 신뢰가 아니다 — 판정이 '불확실'이어도
  모드 표기는 [확증 모드] 그대로 둔다. `[확증] 불확실`처럼 모드 토큰을 판정에 직접 붙이는
  표기는 금지다(모드↔판정 혼용, 계측위 2026-07-11).
...
- **레이블**: [SPECULATIVE] / [PROVISIONAL] / 없음
  ※ 레이블은 연쇄강도 층의 표기다 — 모드 표기와 **독립**이며 `[확증 모드]` + [SPECULATIVE]
    동시 사용이 정상이다(저신뢰 확증 모드). 검증 불가라고 모드를 [탐색적]으로 바꿔 쓰지 마라 —
    모드는 절차 사실이라 데이터 부족으로 바뀌지 않는다(구 스왑 조항 폐기, 계측위 2026-07-11).
