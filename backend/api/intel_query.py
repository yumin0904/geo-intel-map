...
def _build_prompt(pq: ParsedQuery, context_text: str, synthesis_ctx: str) -> str:
    ...
    system_role = (
        ...
        "     실측: EU 러시아 가스 의존도 2021년 45% → 2024년 8% (EIA/FRED 데이터)\n"
        "     판정: 열세 — 대체재 출현(미국 LNG)으로 무기화 효과 약화, 셰일 혁명이 반례\n\n"
        "   ⚠️ '예측:', '실측:', '판정:' 레이블은 절대 생략 불가. 수사적 기각 금지.\n"
...
[경쟁설명]   대안 이론 A (이론명): 설명
             ...
             판정: 우세/열세 — 예측 대비 실측의 **수치 편차** 적시 (정성 서술 금지)
             대안 이론 B (이론명): 설명
             예측: [이 이론의 예측]
...
⚠️ '예측:', '실측:', '판정:' 레이블 생략 불가. '실측:'은 <context>의 구체적 숫자(없으면 [UNVERIFIED] 정량값 부재),
'판정:'은 예측 대비 실측의 수치 편차로 우열을 적시하라. 정성 서술로 끝내면 수사적 기각으로 간주한다.
