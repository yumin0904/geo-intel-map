...
def _build_prompt(pq: ParsedQuery, context_text: str, synthesis_ctx: str) -> str:
    ...
    system_role = (
        ...
        "   ★ <context>에 '경쟁 이론 비교 프로파일' 섹션이 있으면 반드시 활용하라:\n"
        "   - 각 이론의 '반증 가능 예측'과 '실측 데이터'를 비교하여 수치 편차를 제시할 것.\n"
        "   - [경쟁설명] 형식: '예측: [이론 예측 방향] / 실측: [데이터 수치] / 편차: [차이] / 우세: [이론명]'\n"
        "   - 수사적 기각 금지 ('이론 B는 한계가 있다' 수준 불가 — 반드시 수치 근거로 판정).\n\n"
...
"'[가설] 현 데이터로 검증 가능한 정량 가설 없음'으로 표기하고 [검증포인트]에 대안 서술하라.\n\n"
    ...
    _card_fmt = """\
각 인사이트는 아래 카드 형식으로 작성하라. **아래 9개 헤더를 모두 포함할 것 (생략 금지):**
...
[한계]       이 분석이 답하지 못하는 것
[경쟁설명]   대안 이론 A: 설명
             → 예측: [이론 A가 예측하는 방향] / 실측: [수치] / 편차: [차이] / 판정: 우세/열세
             대안 이론 B: 설명
             → 기각 근거: [수치 근거] / 반례: ~할 경우 설명력 하락
[검증포인트] 다음 세션에서 확인할 지표·소스
[문헌공백]   기존 연구가 이 패턴을 충분히 다루지 않는 이유 + 이 분석의 학문적 기여 가능성
```
