...
# ── 루브릭 v2 (T6 채택위 2026-07-10): cr/fa DV-부재 정직성 보정 ──────────────
# 현행 cr 축은 ⓐ대조 작업의 엄밀성과 ⓑ DV 데이터의 가용성을 한 척도에 겹쳐 놓아,
# DV가 원리상 부재한 케이스의 정직한 [UNVERIFIED] 선언·판정거부가 ≤3으로 눌린다
# (실증: v2_rival_dv_absent_honest 골드가 설계 의도대로 행동하고 감점 — 눈금위 07-10).
# 처방 = DV 가용성 선분류 + 분기 앵커. 발효 규약: JUDGE_RUBRIC=v2 명시 시에만 사용,
# 기본은 v1 유지 — 재베이스라인(동결 재채점 A/B + 골드 비대칭 검증) 통과·사용자 승인
# 후에 기본 전환한다 (eval_results/CHANGELOG.md 계측기 거버넌스 절).
_JUDGE_RUBRIC_V2 = """당신은 국제정치학 박사학위 논문 심사위원입니다.
아래 지정학 분석 텍스트를 4개 축으로 각 1~5점 채점하세요. 엄격하되, 아래 단계별 기준을 그대로 적용하십시오.
...
★ 먼저 이 분석의 종속변수(DV) 실측치가 존재하는지 판정한 뒤, 해당 분기의 앵커를 적용하라.
· DV 실측치 존재 = 이론 예측과 대조할 관측 수치(시장가격·공개통계·집계 이벤트수 등)를
  원리상 확보 가능한 경우.
· DV 실측치 부재 = 사건 미발생·폐쇄국가 미관측·데이터 랙 등으로 대조 수치가 원리상 부재한 경우.

〈공통〉
1=단일 이론, 경쟁이론 없음 /
2=경쟁이론 나열하나 수사적 기각('한계가 있다') — 또는 통상 관측가능한 DV(시장가격·공개통계
   등)를 근거 없이 '부재'로 선언하고 대조를 회피(과잉 회피) /

〈DV 실측치 존재 분기〉 — 대조할 수치가 있는데 안 하면 감점
3=경쟁이론 예측은 제시하나 실측 수치 대조 없음 /
4=예측 vs 실측 수치 편차 제시. 단 제시 수치에 데이터 출처가 명기돼 검증가능할 것
   (출처 없는 수치·산술은 날조 위험 → 3점 이하로 감점) /
5=양 이론 예측 vs 실측 편차를 출처 있는 수치로 명확히 제시 + 연구자가 우열을 판단하는 데
   필요한 쟁점(숨은 가정·범위조건·metric 상이)을 정확히 제시

〈DV 실측치 부재 분기〉 — 정직한 부재 선언이 정답. 수치를 지어내면 오히려 최하위
3=부재를 인지하나 사유·대체경로 없이 '판정 불가'로만 종결 / 또는 부재 이론블록을 근거 없이
   우세·열세로 판정(침묵을 실측으로 위조 — [UNVERIFIED] 옆에 판정 발명) /
4=DV 부재를 [UNVERIFIED]로 명시하고 우열 판정을 거부 + 부재의 구체적 사유(사건 미발생·구조적
   미관측·데이터 랙 중 무엇인지)를 특정 /
5=위 4 + 부재를 해소할 대체 검증경로를 구체 특정(어떤 데이터·소스·관측시점이 갖춰지면 어느
   이론이 검정 가능한지). 실측치를 지어내지 않고 검정 조건을 설계로 넘긴 상태

   ※ [9-Q 해석 주체 원칙] AI가 '어느 이론이 우세한가'를 대신 단정하지 않는 것이 **정상**이다.
     두 분기 모두, 편차(사실)나 부재(사실)를 정확히 제시하고 판단 쟁점을 연구자에게 넘겼다면
     감점하지 말 것. '판정 참여'란 우열 단정이 아니라 대조 구조(편차·쟁점) 제시를 뜻한다.
   ※ [게이밍 방어] DV가 통상 관측가능한데도 부재를 선언하면 회피이므로 2점 이하(공통 2).
     반대로 DV가 원리상 부재한데 수치를 제시하면 날조이므로 3점 이하(존재 분기 4 단서).
     정직한 부재 선언(부재 사유 + 대체경로 특정)만 4~5점에 도달한다.

[falsifiability 반증가능성]
1=측정불가 추상 주장 / 2=가설 있으나 변수 모호 / 3=H1 있으나 측정·통제변수 불완전 /
4=측정가능 변수 + 통제변수 명시한 H1 /
5=반증가능 H1 + 검정 방법·데이터까지 특정. 이때 특정된 데이터가 현재 존재하지 않아도
   (사건 미발생·미관측·데이터 랙) 검정 설계로서 데이터·방법·판정 임계가 명시되면 5점 자격이다.
   스스로 반증 조건(어떤 관측이 나오면 H1이 기각되는가)을 명시한 가설은 그 데이터가 아직
   없더라도 Popper 의미의 반증가능성을 온전히 충족한다.
   ※ [부재 게이밍 방어] '검정 불가'로만 종결하고 어떤 데이터·방법이 갖춰지면 검정되는지
     특정하지 않으면 4점 이상 불가 — 막연한 회피와 설계된 유예를 구분한다.
...
# 루브릭 선택 — 기본 v1. v2 전환은 재베이스라인 절차·사용자 승인 후(위 발효 규약).
...
def _judge_quality(full_text: str, rubric_text: str | None = None) -> dict | None:
    ...
    rubric_text: 명시 시 해당 루브릭으로 채점(A/B 재채점용) — 미지정이면 버전 선택 규칙.
    """
    if not full_text:
        return None
...
def main() -> None:
    ...
    report = {
        ...
        # 루브릭도 계측기 신원의 일부 — v1↔v2 점수는 절대 비교 불가(T6 채택위 2026-07-10).
        "judge_rubric": _JUDGE_RUBRIC_VERSION,
        "total": len(results),
