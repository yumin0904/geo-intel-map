...
def _confidence_score(
    ...
    if controlled:
        score += 15
    # ⚠️ [B25 수리 2026-07-14] 구 주석은 "80개+ → 통계력 충분"이라 적혀 있었다. **거짓이다.**
    #    현실적 효과(f²=0.0045)에서 n=80의 달성 검정력은 **0.091 — 9%**다.
    #    80% 검정력에 필요한 표본은 **1,750개**(≈6.9년)이고, 이 엔진의 최대 창(411일)에서도
    #    0.274에 그친다. 즉 이 가점은 "통계력이 충분해서" 주는 것이 아니라
    #    **"표본이 하한을 넘었으니 최소한 검정이 성립한다"**는 뜻일 뿐이다.
    #    진짜 검정력은 services/statistical_power.py가 계산하고,
    #    부족하면 verification_status가 UNDERPOWERED로 그 사실을 드러낸다.
    if n_obs >= _D4_THRESHOLD * 2:  # 80개+ → 검정 성립 하한의 2배 (검정력 '충분'이 아니다)
        score += 10
