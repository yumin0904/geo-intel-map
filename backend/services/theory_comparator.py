...
def build_theory_comparison_context(
    ...
    lines.append(
        ...
        "  ⚠️⚠️ [절대 금지 — affirming the null] context에 기록이 없다는 사실(침묵)을\n"
        "     '양보 없음'·'차질 없음'·'0% 변화'·'0건' 같은 실측값으로 변환하지 마라. 부재는\n"
        "     측정이 아니다. 그렇게 위조한 값으로 우세/열세를 찍지 마라. DV 부재면\n"
        "     판정은 반드시 '전제충족(DV 미검증)'으로 종결 — H1이든 경쟁이론이든 동일 규칙.\n"
        "  ⚠️ [정성 반대증거의 처리] 대응 조치 등 '정성 관찰'이 예측과 상반되면 그것은 기록하라 —\n"
        "     단 '정성 증거: …(예측과 상반)' 형태로 별도 기재하고, 건수·수치로 변환하거나\n"
        "     우세/열세 판정의 근거로 쓰지 마라. 판정 줄은 여전히 '전제충족(DV 미검증)'이다.\n"
        "     예: 판정: 전제충족(DV 미검증) — 정성 증거(해군 증파)는 양보 방향과 상반\n"
    )
